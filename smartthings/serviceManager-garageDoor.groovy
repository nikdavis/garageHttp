 ...
 */
definition(
		name: "Garage Door Service Manager",
		namespace: "nikdavis",
		author: "Nikolas Davis, SmartThings",
		description: "This manages the discovery and installation of your garage door device.",
...
preferences {
	page(name: "searchTargetSelection", title: "UPnP Search Target", nextPage: "deviceDiscovery") {
		section("Search Target") {
			input "searchTarget", "string", title: "Search Target", defaultValue: "urn:nivvis-co:device:garageDoor:0-1", required: true
		}
	}
...
def deviceDiscovery() {
	...
	devices.each {
		def value = it.value.name ?: "UPnP Device ${it.value.ssdpUSN.split(':')[1][0..7]}"
	...
	selectedDevices.each { dni ->
		...
		if (selectedDevice) {
			d = getChildDevices()?.find {
				it.deviceNetworkId == "${selectedDevice.value.mac}-opener" ||
                	it.deviceNetworkId == "${selectedDevice.value.mac}-light"
			}
		}

		if (!d) {
        	def openerDni = "${selectedDevice.value.mac}-opener"
        	def lightDni = "${selectedDevice.value.mac}-light"
			log.debug "Creating children for UPnP Device with dni: ${selectedDevice.value.mac}"

			addChildDevice("nikdavis", "Garage Door Opener", openerDni, selectedDevice?.value.hub, [
            ...
            addChildDevice("nikdavis", "Garage Door Light", lightDni, selectedDevice?.value.hub, [
